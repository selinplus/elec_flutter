...
import 'package:remote_hospital/components/md_prescription/md_pres_component.dart';
import 'package:remote_hospital/services/pref_util.dart';
class HistoryTodayPresListComponent extends StatefulWidget {
//  Mendian mendian;
//  MdPresListComponent({this.mendian});
  @override
  _HistoryTodayPresListComponentState createState() => _HistoryTodayPresListComponentState();
}

class _HistoryTodayPresListComponentState extends State<HistoryTodayPresListComponent> {
  List<Prescription> prescriptions =List<Prescription>();
  Mendian mendian =Mendian.name();
  void _getPres() {
    Prescription _p;
    Map<String,int> params = Map<String,int>();
    // params["ID"]=widget.mendian.ID;
    int mdid;
//     mdid= await getPrefString("mdid");
//     print("mdid:"+mdid);
//     params["id"]=3;
    getPrefString("yaosshuri").then((val){
      ...
      DioUtil().get(true,"/api/v1/prescription/cfuri",queryParameters: params,errorCallback: (statuscode){
      ...
      child:Container(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (BuildContext context,int index){
            return MdPresComponent( mendian:mendian,prescription:prescriptions[index],refresh:_getPres,);
          },

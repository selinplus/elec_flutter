...
class AccountLoginComponent extends StatelessWidget {
  ...
  final callback;
  AccountLoginComponent({this.label,this.md,this.callback});
    ...
    return GestureDetector(
        onTap: (){
          Navigator.push(context, MaterialPageRoute(builder: (context) {
            //指定跳转的页面
            return EditPassPage(md: md,callback: (val)=>_onPasswordChange(val));
          },));
        },
      child:  Container(
          child:Row(
...
//             color: Colors.blue,
                    ...
                    height: 40.0,
                    child: Text("${md.username}", style: TextStyle(fontSize: 14),
                    ),
                  )//
              ),
              Container(
...
class EditPassPage extends StatelessWidget{
   ...
   final callback;
   EditPassPage({this.md,this.callback});
